...
//        ig.main('#canvas', OpenScreen, 60, 512, 384, 1.5);
        ...
        function start() {
          // checking rotation per http://www.w3.org/TR/screen-orientation/
          if ('orientation' in screen &&
              screen.orientation.indexOf('portrait') === 0) {
            screen.addEventListener('orientationchanged', function rotated(e) {
              screen.removeEventListener('orientationchanged', rotated);
              start();
            });
            return;
          }
          ...
          var targetHeight = Math.min(window.innerWidth, window.innerHeight);

          var scale = Math.min(targetWidth / 512, targetHeight / 384);
          ig.main('#canvas', OpenScreen, 25, 512, 384, scale);
        }
        start();
    });
